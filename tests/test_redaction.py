...
def test_redact_obj_scrubs_pii_embedded_in_strings():
    out = redact_obj({
        "url": "https://egov.uscis.gov/casestatus/IOE0935749409/x",
        "note": "no identifiers here",
    })
    assert "IOE0935749409" not in out["url"]
    assert REDACTION_MASK in out["url"]
    assert out["note"] == "no identifiers here"


def test_redact_obj_is_pure():
    src = {"receiptNumber": "IOE0935749409", "events": [{"code": "ABC"}]}
    out = redact_obj(src)
    assert src["receiptNumber"] == "IOE0935749409"  # unchanged
    assert out["receiptNumber"] == REDACTION_MASK
    assert out["events"] is not src["events"]


def test_scrub_text_patterns():
    assert scrub_text("case IOE0935749409 here") == f"case {REDACTION_MASK} here"
